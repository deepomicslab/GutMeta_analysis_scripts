...
# viersion 1.2.1 author: Yiqi Jiang
# support sample name with number
...
def check_sample_name(data):
    for sample in data.columns:
        if sample[0].isdigit():
            new = 'sample_{}'.format(sample)
            data.rename(columns = {sample:new}, inplace = True)
    return data
...
def metadata2gf(metadata,group_id='phenotype'):
    ...
    data = check_sample_name(data.T).T
...
def get_merged(ifile1,ifile2):
    ...
    merged_df = check_sample_name(merged_df)
