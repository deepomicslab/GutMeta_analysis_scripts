...
class Transfer_times():
    ...
    def read_events(self, identified_hgt):
        ...
        for line in open(identified_hgt):
            ...
            if array[1] == "sample":
                continue
            event = Event(array, 'HGT_c{}'.format(i+1))
            sample = array[1]
        ...
        for sample in trans.HGT_event_dict:
            for event in trans.HGT_event_dict[sample]:
                if event.reverse_flag == False:
                    ...
                    data.append([event.id, sample, from_ref, from_bkp, event.reverse_flag, from_bkp-args["seq_len"], from_bkp+args["seq_len"],from_seq, \
