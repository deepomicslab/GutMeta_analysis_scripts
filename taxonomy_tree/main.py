...
ops, args = getopt.getopt(sys.argv[1:], '', ['abdf=', 'dsf=', 'ann=', 'method=', 'groupid=',"adjust=", "outdir="])
for op, arg in ops:
    if op == '--abdf':
        ifile1 = arg
    if op == '--dsf':
...
os.system("head -1 {}|sed 's/^s/ID/' > {}".format(os.path.join(outdir, 'output.diff_testing.wilcox.test.s.stat.tsv'), os.path.join(outdir, 'output.taxonomy_tree.testing.tsv')))
os.system("for i in `ls {}`; do sed 1d $i >> {};done".format(os.path.join(outdir, 'output.diff_testing.wilcox.test.*stat.tsv'), os.path.join(outdir, 'output.taxonomy_tree.testing.tsv')))
os.system("rm {}*".format(os.path.join(outdir, 'output.diff_testing')))