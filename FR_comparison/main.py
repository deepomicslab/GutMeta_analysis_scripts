...
nfr_df = pd.DataFrame(index=sp_df.columns, columns=['sample','group', 'nFR'])
...
#p_df.loc['nFR', 'g1_occ'] = len(g1_v[g1_v > 0])/len(g1_v)
#p_df.loc['nFR', 'g2_occ'] = len(g2_v[g2_v > 0])/len(g2_v)
p_df.loc['nFR', 'g1_n'] = len(g1_v)
p_df.loc['nFR', 'g2_n'] = len(g2_v)

if method == 't.test':
    p_df.loc['nFR', 'p_value'] = ttest_ind(g1_v, g2_v)[1]
elif method == 'wilcox.test':
    p_df.loc['nFR', 'p_value'] = mannwhitneyu(g1_v, g2_v)[1]
elif method == 'kruskal.test':
    p_df.loc['nFR', 'p_value'] = kruskal(g1_v, g2_v)[1]
elif method == 'aov':
    p_df.loc['nFR', 'p_value'] = f_oneway(g1_v, g2_v)[1]
else:
