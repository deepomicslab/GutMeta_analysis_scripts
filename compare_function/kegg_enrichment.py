"""
Given KO list, perform KEGG enrichment analysis
"""
...
def get_pathways(input_ko_ids, ko_pathway_dict):
...
def get_pathway_name_class_static(pathway_id, name_table):
    if pathway_id in name_table.index:
        return name_table.loc[pathway_id, ]
    else:
        return 'Unknown', 'Unknown', 'Unknown'

def enrichment_analysis(input_ko_ids, background_ko_ids, input_counts, background_counts, output):
    data = []
    ...
    # else:
    #     print (f"pathway num: {len(df)}, enriched num is {enriched_num}, depleted num is {depleted_num}, depleted ratio is NAN" )

def enrichment_analysis_static(input_ko_ids, background_ko_ids, input_counts, background_counts, output, name_table):
    data = []
    # Perform a Fisher's exact test for each pathway
    for pathway_id in set(input_counts.keys()) | set(background_counts.keys()):
        # print (pathway_id)
        ...
        #     continue
        # print(p_value, 'KEGG')
        pathway_name, first_class, second_class = get_pathway_name_class_static(pathway_id, name_table)
...
#        ko_pathway_dict = pickle.load(f)
    ...
    ################## these KO lists are generated by analyze_transfer_gene.py	

    # input_ko_ids = read_list("/mnt/d/HGT/seq_ana/transfer_ko.txt")
    # background_ko_ids = read_list("/mnt/d/HGT/seq_ana/no_transfer_ko.txt")
    # output = "/mnt/d/R_script_files/transfer_enriched.csv" # to store enriched pathways
    ...
    # output = "/mnt/d/R_script_files/insert_enriched.csv" # to store enriched pathways
    # input_counts = get_pathways(input_ko_ids)
    # background_counts = get_pathways(background_ko_ids)
    # enrichment_analysis(input_counts, background_counts)
    ...
    # output = "/mnt/d/R_script_files/bkp_enriched.csv" # to store enriched pathways
    # input_counts = get_pathways(input_ko_ids)
    # background_counts = get_pathways(background_ko_ids)
    # enrichment_analysis(input_counts, background_counts)
